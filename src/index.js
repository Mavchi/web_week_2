...
function initializeCode() {
  ...
  submitButton.addEventListener("click", function (event) {
    ...
    const tableLinesElements = document.querySelectorAll("tbody tr");

    // let's find out if given username already exists in our database-set
    let usernameAlreadyIncluded = false;
    for (let i = 0; i < tableLinesElements.length; i++) {
      if (
        tableLinesElements[i].querySelector("td").innerText ===
    ...
    if (usernameAlreadyIncluded) {
      for (let i = 0; i < tableLinesElements.length; i++) {
        ...
        if (
          ...
          allComponents[3].innerText =
            adminElement.checked === true ? "X" : "-";
        }
      }
    } else {
